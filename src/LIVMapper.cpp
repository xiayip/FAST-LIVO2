...
void LIVMapper::initializeSubscribersAndPublishers(rclcpp::Node::SharedPtr &node, image_transport::ImageTransport &it_)
{
  ...
  if (p_pre->lidar_type == AVIA) {
    sub_pcl = this->node->create_subscription<zephyr_msgs::msg::CustomMsg>(lid_topic, 200000, std::bind(&LIVMapper::livox_pcl_cbk, this, std::placeholders::_1));
  } else {
...
void LIVMapper::livox_pcl_cbk(const zephyr_msgs::msg::CustomMsg::ConstSharedPtr &msg_in)
{
  if (!lidar_en) return;
  mtx_buffer.lock();
  zephyr_msgs::msg::CustomMsg::SharedPtr msg(new zephyr_msgs::msg::CustomMsg(*msg_in));
